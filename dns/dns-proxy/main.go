...
func parse(filename string) (map[string]string, error) {
	...
	for scanner.Scan() {
		...
		if len(parts) < 2 {
			return records, fmt.Errorf("%s not a valid line", line)
		}
...
func main() {
	...
	dns.HandleFunc(".", func(w dns.ResponseWriter, req *dns.Msg) { //anon func, single use
		if len(req.Question) < 1 {
			dns.HandleFailed(w, req)
			return
		}
		fqdn := req.Question[0].Name
		log.Printf("Got request for %v\n", fqdn)
